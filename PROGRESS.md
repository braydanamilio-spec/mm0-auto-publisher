...
- **[máy 2 — 22/8] KÊNH IMPORT (video làm sẵn từ trước, tách riêng khỏi render)**: thêm 2 kênh `BEYOND` (ch5, space/deep-time) + `LEGACY` (ch6, tiểu sử) vào channels.yaml + brands.json (section riêng, `imported:true`, FB tắt); `ch7_datarace` = kênh RIÊNG user làm trên máy cũ (KHÔNG phải kênh DATARACE hệ thống) -> tạo key mới `RANKRUSH` (RANK RUSH, @rankrush, accent #22D3EE), script tự rebrand 'Data Race'/#datarace -> 'Rank Rush'/#rankrush trong title+desc. Script nạp kho: `scripts/import_legacy.py` (dry-run không cần creds; chạy thật cần GOOGLE_APPLICATION_CREDENTIALS + kho pool; idempotent nhờ dedup, resume `--start`). Kiểm kê: 907 video (BEYOND 30L+90S · LEGACY 44L+134S · RANKRUSH 156L+453S gồm compilations). CHƯA upload (máy 2 không có sa.json); cần kết nối YouTube cho BEYOND/LEGACY/RANKRUSH trên dashboard (label đúng tên key) trước khi hệ đăng thật.
